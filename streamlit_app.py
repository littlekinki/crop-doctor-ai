...
def generate_comprehensive_batch_report(results, batch_timestamp):
    ...
    report_lines.append("=" * 80)
    report_lines.append("")
    report_lines.append("Report generated by Crop Doctor - AI-Powered Crop Disease Diagnosis and Treatment Recommendation")
...
""", unsafe_allow_html=True)
    ...
    # Buttons for options
    opt_cols = st.columns(min(num_predictions + 4, 6))
    for i in range(1, min(num_predictions + 5, 7)):
        with opt_cols[i-1]:
            if st.button(f"{i}", key=f"batch_opt_{i}", use_container_width=True):
                if i == num_predictions + 4:
                    st.session_state.show_batch_results = False
                    st.session_state.batch_results = None
                    st.session_state.batch_mode = False
                    st.rerun()
                elif i == num_predictions + 3:
    ...
    if st.session_state.get('show_k_dialog', False):
        with st.expander("📊 Change number of top predictions", expanded=True):
            ...
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Set", width="stretch", key="batch_set_k_btn"):
                    st.session_state.current_top_k = new_k
                    st.session_state.show_k_dialog = False
                    st.session_state.show_results = False
                    st.rerun()
            with col2:
                if st.button("❌ Cancel", width="stretch", key="batch_cancel_k_btn"):
                    st.session_state.show_k_dialog = False
                    st.rerun()
    
    # ============================================================
    # 22. VIEW ALL BATCH RESULTS (MOVED BEFORE FEEDBACK)
    # ============================================================
    ...
    # ============================================================
    # 24. FEEDBACK SECTION (MOVED AFTER BATCH RESULTS)
    # ============================================================
    st.markdown("---")
