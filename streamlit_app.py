...
def export_batch_detailed_report(results):
    ...
    # Create detailed JSON report with proper type conversion
    detailed_report = []
    for r in results:
        if r['status'] == 'success':
            # Convert top_predictions to native types
            top_preds_native = []
            for pred in r['top_predictions']:
                top_preds_native.append({
                    "class": pred['class'],
                    "confidence": float(pred['confidence'])  # Convert numpy float to Python float
                })
            
            report_entry = {
                "filename": r['filename'],
                "diagnosis": r['primary_diagnosis'],
                "confidence": float(r['primary_confidence']),  # Convert numpy float
                "category": r['category'],
                "causal_agent": r['causal_agent'],
                "management": r['management'],
                "chemical_control": r['chemical_control'],
                "is_healthy": bool(r.get('is_healthy', False)),  # Convert to bool
                "top_predictions": top_preds_native,
                "timestamp": r.get('timestamp', timestamp)
            }
            detailed_report.append(report_entry)
    
    # Save as JSON with type conversion
    json_filename = f"batch_detailed_report_{timestamp}.json"
    with open(json_filename, 'w', encoding='utf-8') as f:
        json.dump(detailed_report, f, indent=2, ensure_ascii=False, default=convert_to_native)
